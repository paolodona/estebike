...
const LIMIT =
  LIMIT_IDX >= 0 ? parseInt(process.argv[LIMIT_IDX + 1], 10) : Infinity;
const ZIP_PATH = process.argv[2];

if (!ZIP_PATH || ZIP_PATH.startsWith('--')) {
  console.error(
    'Usage: node import-export.mjs <path-to-zip> [--dry-run] [--limit N]'
  );
  process.exit(1);
}
...
async function main() {
  ...
  for (let fi = 0; fi < allFiles.length; fi++) {
    ...
    const { score, reasons } = await scoreNonPhoto(filePath, filename, caption);
